    ...
    try {
      ...
      if (res.ok) {
        ...
        if (price && typeof price === 'number') {
          ...
          if (window.staticData?.recommendations) {
            window.staticData.recommendations.forEach(r => {
              if (r.ticker === ticker) r.cmp = price;
            });
          }
          if (window.staticData?.watchlist) {
            window.staticData.watchlist.forEach(w => {
              if (w.ticker === ticker) {
                w.cmp = price;
          ...
          if (window.staticData?.stock_fo_setups) {
            window.staticData.stock_fo_setups.forEach(f => {
...
function updateSyncTimeBadge() {
  ...
  const dateStr = window.staticData?.date || new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' });
...
function setTimeframeFilter(tf, btn) {
  ...
  if (headingText) {
    ...
    else if (tf === '1_month') headingText.textContent = '⚡ 1-Month Stock Swings (Weekly Horizon)';
    else if (tf === '3_month') headingText.textContent = '🚀 3-Month Positional Swings (Weekly Horizon)';
